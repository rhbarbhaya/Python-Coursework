...
for i in cleaned_pro:
    try:
        ...
        try:
            ...
            try:
                sentiment_pos = swn.senti_synset(i + '.a.03').pos_score()
                Positive_Score_Pro += sentiment_pos
                sentiment_con = swn.senti_synset(i + '.a.03').neg_score()
                Negetive_score_Pro += sentiment_con
            except:
                try:
                    sentiment_pos = swn.senti_synset(i + '.s.03').pos_score()
                    Positive_Score_Pro += sentiment_pos
                    sentiment_con = swn.senti_synset(i + '.s.03').neg_score()
                    Negetive_score_Pro += sentiment_con
                except:
                    try:
                        sentiment_pos = swn.senti_synset(i + '.r.03').pos_score()
                        Positive_Score_Pro += sentiment_pos
                        sentiment_con = swn.senti_synset(i + '.r.03').neg_score()
                        Negetive_score_Pro += sentiment_con
                    except:
                        continue
...
for j in cleaned_con:
    try:
        ...
        try:
            ...
            try:
                sentiment_pos = swn.senti_synset(j + '.a.03').pos_score()
                Positive_Score_Pro += sentiment_pos
                sentiment_con = swn.senti_synset(j + '.a.03').neg_score()
                Negetive_score_Con += sentiment_con
            except:
                try:
                    sentiment_pos = swn.senti_synset(j + '.s.03').pos_score()
                    Positive_Score_Con += sentiment_pos
                    sentiment_con = swn.senti_synset(j + '.s.03').neg_score()
                    Negetive_score_Con += sentiment_con
                except:
                    try:
                        sentiment_pos = swn.senti_synset(j + '.r.03').pos_score()
                        Positive_Score_Con += sentiment_pos
                        sentiment_con = swn.senti_synset(j + '.r.03').neg_score()
                        Negetive_score_Con += sentiment_con
                    except:
                        continue

#Printing the bigrams are
print "---------------------------"
print "Some bigrammed pros are:\n", bigrammed_pro[:5]
print "---------------------------"
print "Some bigrammed cons are:\n", bigrammed_cons[:5]
print "---------------------------"
